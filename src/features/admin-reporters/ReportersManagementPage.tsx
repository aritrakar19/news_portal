  ...
  return (
    ...
    >
      <motion.div variants={itemVariants} className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-heading font-extrabold text-slate-900 dark:text-white">{t("admin.reporters.title", "Reporters Management")}</h1>
          <p className="text-muted-foreground mt-2 max-w-2xl">
            {t("admin.reporters.subtitle", "Manage your network of reporters, review applications, and monitor activity.")}
          </p>
        </div>
        <div className="flex gap-3">
          <Button variant="outline">
            <Settings className="mr-2 h-4 w-4" /> {t("admin.reporters.config", "Config")}
          </Button>
          <Button className="bg-blue-600 hover:bg-blue-700 text-white">
            <UserPlus className="mr-2 h-4 w-4" /> {t("admin.reporters.invite", "Invite Reporter")}
          </Button>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        <motion.div variants={itemVariants} className="xl:col-span-2 space-y-6">
          <div className="bg-slate-50/50 dark:bg-slate-900/50 p-6 rounded-xl border">
            <h2 className="text-xl font-semibold mb-4">{t("admin.reporters.directory", "All Reporters Directory")}</h2>
            <ReportersFilters />
