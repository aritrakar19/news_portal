    ...
    <Card className="border-border shadow-sm h-full">
      <CardHeader className="pb-2">
        <CardTitle className="text-lg font-heading flex items-center justify-between">
          <span>{t("dashboard.profile.title", "Reporter Profile")}</span>
          <Badge className="bg-green-100 text-green-700 hover:bg-green-200 border-none">
            <CheckCircle2 className="w-3 h-3 mr-1" /> {t("dashboard.profile.verified", "Verified")}
          </Badge>
        </CardTitle>
      </CardHeader>
      <CardContent>
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">{t("dashboard.profile.role", "National Correspondent")}</p>
        ...
        <div className="space-y-2 mb-6">
          <div className="flex justify-between text-sm font-medium">
            <span>{t("dashboard.profile.completion", "Profile Completion")}</span>
        ...
        <div className="bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/50 rounded-lg p-3 flex gap-3 text-sm">
          ...
          <div>
            <p className="font-semibold text-amber-800 dark:text-amber-400">{t("dashboard.profile.addBank", "Add Bank Details")}</p>
            <p className="text-amber-700/80 dark:text-amber-500/80 mt-0.5">{t("dashboard.profile.addBankDesc", "Please add your bank account to enable direct withdrawals.")}</p>
          </div>
        </div>
      </CardContent>
    </Card>
